...
- 답: 
    + 1번 : 하위 branch와 상위 branch를 공통 부모로 갖는 새로운 commit을 상위 branch에 생성한다.
    + 2번 : 하위 branch에서 생성된 commit들을 합친 새 commit을 상위 branch에 추가한다.
    + 3번 : 하위 branch에 있는 commit들을 commit hash를 바꿔 모두 상위 branch로 옮긴다.
...
- 답: 
    1. 맨 처음 commit id의 첫 6글자는 ```a1s2d3f```다.   
    2. 현 branch는 develop이다.
    3. 맨 처음에 ```docs: readme 추가```라고 commit 했다.
